...
def hunt_detail(
    ...
    return render(request, "hunts/detail.html", ctx)
...
def add_hunt_bonus(
    ...
    if game.strip() and bet_dec and bet_dec > 0 and win_dec is not None and win_dec >= 0:
...
def close_hunt(
    ...
    convention = (
        end_convention if end_convention in ("after_opening", "spin_end") else "after_opening"
    )
    hunt_svc.close_hunt(
        session,
        hunt_id=hunt_id,
        end_balance=parse_decimal(end_balance, places=2),
        end_convention=convention,
    )
    session.commit()
    return RedirectResponse(f"/hunts/{hunt_id}", status_code=303)
